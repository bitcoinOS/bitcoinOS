...
use crate::domain::{PointRecord, StakingPoolInfo};
...
pub async fn serve() {
    ...
    for p in stake_pools {
        ic_cdk::print(format!("in  stake pool {p:?}\n"));
                ...
                Ok(v) => {
                    for s in v {
                        ...
                        if let Some(ammout) = total_stake_point_per_user.get(&sender) {
                            total_stake_point_per_user
                                .insert(sender.clone(), ammout + s.sent_amount);
                        } else {
