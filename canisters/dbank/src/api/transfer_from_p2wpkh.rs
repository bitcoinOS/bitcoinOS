...
use wallet::domain::{EcdsaKeyIdAndDerivationPath, Wallet};
...
pub(super) async fn serve(
    ...
    let p2wpkh_wallet: Wallet =
        repositories::wallet::get_p2wpkh_wallet(&metadata.clone(), wallet_owner)
            .map(|raw| raw.into())
            .ok_or_else(|| DBankError::WalletNotFound(wallet_owner.to_string()))?;
