...
const WALLET_INFO_MEMORY_ID: MemoryId = MemoryId::new(6);
const WALLET_LOG_IDX_MEM_ID: MemoryId = MemoryId::new(7);
const WALLET_LOG_DATA_MEM_ID: MemoryId = MemoryId::new(8);
...
pub(super) fn get_sequencer_memory() -> Memory {
...
pub(super) fn get_wallet_log_index_memory() -> Memory {
    MEMORY_MANAGER.with(|m| m.borrow().get(WALLET_LOG_IDX_MEM_ID))
}

pub(super) fn get_wallet_log_data_memory() -> Memory {
