...
deploy_staking:
    ...
    dfx deploy stakingpool --argument "(record { name = \"stakingpool\"; network = variant { regtest }; os_canister = principal \"$OS_CANISTER\"; description = \"a staking pool with 10 annual interest rate for a year\"; annual_interest_rate = 10; duration_in_day = 1; })"
...
deploy_point:
    ...
    OS_CANISTER=$(dfx canister id os)
    echo "${OS_CANISTER}"
    dfx deploy point  -m reinstall --argument "(record { network = variant { regtest }; os_canister = principal \"${OS_CANISTER}\"; steward_canister = principal \"aaaaa-aa\";task_period=60 })"
